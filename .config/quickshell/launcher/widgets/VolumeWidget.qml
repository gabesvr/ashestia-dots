...
Item {
    ...
    signal volumeChangeRequested(real pct)
    ...
    Item {
        ...
        Item {
            ...
            Item {
                ...
                Item {
                    ...
                    anchors.bottom: parent.bottom
                    height: parent.height * Math.min(1.0, Math.max(0.0, root.volumeVal))
                    clip: true
                    visible: height > 0
                ...
                // Apple Bottom Icon (Speaker) with dynamic contrast inversion
                Image {
                    id: vIconLight
                    anchors.bottom: parent.bottom
                    anchors.bottomMargin: 14
            ...
            Column {
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        anchors.top: parent.top
                        anchors.bottom: parent.bottom
                        width: Math.max(0, parent.width * Math.min(1.0, Math.max(0.0, root.volumeVal)))
                        radius: 11
                        color: root.isMuted ? Qt.rgba(1, 1, 1, 0.25) : Qt.rgba(1.0, 1.0, 1.0, 0.88)
        ...
        MouseArea {
            ...
            onDoubleClicked: {
                root.toggleMuteRequested();
            }

            function updateFromMouse(posX, posY) {
                let pct = 0;
                if (root.isVertical) {
                    // Vertical slider: maps smoothly to track area
                    let trackTop = 38;
                    let trackHeight = Math.max(1, full.height - 52);
                    let relativeY = posY - trackTop;
                    pct = Math.max(0.0, Math.min(1.0, 1.0 - (relativeY / trackHeight)));
                } else {
                    pct = Math.max(0.0, Math.min(1.0, posX / full.width));
                }
                root.volumeChangeRequested(pct);
            }
        }
    }
}
